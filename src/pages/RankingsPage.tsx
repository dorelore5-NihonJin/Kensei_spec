...
import { Trophy, Cpu, Zap, Search, Layers, Scale, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function RankingsPage({ cpus, gpus }: RankingsPageProps) {
  ...
  // Calculate sorted CPU ranking list (descending score)
  ...
  // Calculate sorted GPU ranking list (descending score)
  ...
  // Filter list by search query and manufacturer
  ...
  }, [activeList, searchQuery, manufacturerFilter]);
  ...
  return (
    <div className="max-w-[1400px] w-full mx-auto px-4 sm:px-6 py-4 flex flex-col gap-8 animate-fadeIn">
      ...
      {/* 2. SEARCH & MANUFACTURER FILTER BAR */}
      <div className="bg-white dark:bg-[#1A1C1E] border border-black/10 dark:border-white/10 rounded-3xl p-5 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        {/* Search Input Box */}
      ...
      <div className="bg-white dark:bg-[#1A1C1E] border border-black/10 dark:border-white/10 rounded-3xl p-6 shadow-xl flex flex-col gap-4">
        {paginatedList.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {paginatedList.map((item, pageIdx) => {
              const globalRank = (currentPage - 1) * itemsPerPage + pageIdx + 1;
